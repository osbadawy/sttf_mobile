...
export function useMultiPlayerWhoopData({ 
  ...
  const [selectedPlayerMetrics, setSelectedPlayerMetrics] = useState<MultiDayWhoopMetrics>({});
  const [primaryLoading, setPrimaryLoading] = useState(false);
  const [selectedPlayerLoading, setSelectedPlayerLoading] = useState(false);
  const [primaryError, setPrimaryError] = useState<string | null>(null);
  const [selectedPlayerError, setSelectedPlayerError] = useState<string | null>(null);

  const fetchPlayerData = useCallback(async (firebaseId: string, isSelectedPlayer = false) => {
    console.log("fetchPlayerData called with:", { firebaseId, isSelectedPlayer, user: !!user });
    if (!user || !firebaseId) {
    ...
    console.log("Cache key:", cacheKey, "Has cache:", dataCache.has(cacheKey));
  ...
  useEffect(() => {
    console.log("useMultiPlayerWhoopData - selectedPlayerFirebaseId:", selectedPlayerFirebaseId);
    if (selectedPlayerFirebaseId) {
      console.log("Fetching selected player data for:", selectedPlayerFirebaseId);
