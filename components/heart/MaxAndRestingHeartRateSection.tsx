...
function Bar({
  ...
  const maxPossibleHeight =
    200 - Math.abs(minVal - item.resting - circleWidth - 10);
  const scaleFactor = Math.min(1, maxPossibleHeight / 200);

  const height =
    ((item.max - item.resting) / (maxVal - minVal)) * 200 * scaleFactor;
...
function Graph({
  ...
  const zip = (a: HeartRateHistory[], b: HeartRateHistory[]) =>
    a.map((k, i) => [k, b[i]]);
  const history = zip(p1history, p2history);

  return (
    <View className="h-[200px] w-full flex-row flex justify-center items-end">
      {history.map((items, index) => {
        ...
        return (
          <View
            className={`flex-row h-[200px] justify-center items-end`}
            key={index}
            style={{
...
export default function MaxAndRestingHeartRateSection({
  ...
  const p1FliteredHistory = p1History.filter(
    (item) => item.max !== 0 && item.resting !== 0,
  );
  const p2FliteredHistory = p2History?.filter(
    (item) => item.max !== 0 && item.resting !== 0,
  );
