...
export default function MaxAndRestingHeartRateSection({
  ...
  const averageMax =
    history.map((item) => item.max).reduce((a, b) => a + b, 0) / history.length;
  const averageResting =
    history.map((item) => item.resting).reduce((a, b) => a + b, 0) /
    history.length;
    ...
    >
      ...
      <View className="h-[200px] w-full flex-row flex justify-between items-end">
        {history.map((item, index) => {
          ...
          const gradientColors = isLatest
            ? [colors.heart, colors.heartResting]
            : [colors.heartLight, colors.heartRestingLight];
          ...
          return (
            ...
            >
              {/* Top circle */}
              <View
                className={`w-4 h-4 rounded-full bg-white mb-[-1px] border-[3px] border-solid ${isLatest ? "border-heart" : "border-heartLight"}`}
              />
              ...
              {/* Bottom circle */}
              <View
                className={`w-4 h-4 rounded-full bg-white mt-[-1px] border-[3px] border-solid ${isLatest ? "border-heartResting" : "border-heartRestingLight"}`}
              />
            </View>
          );
        })}
