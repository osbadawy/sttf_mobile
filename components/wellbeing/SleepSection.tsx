...
export default function SleepSection({rem, sws, light, awake, score}: SleepSectionProps) {
    ...
    return (
        <CardWithTitle title={t('title')} icon={<SleepIcon />} titleColor="text-sleep" isRTL={isRTL} arrow={false}>
            <View className="flex-row items-center justify-center py-4">
                ...
                <View className="relative">
                    ...
                    <View className="absolute inset-0 items-center justify-center">
                        <Text className="text-4xl font-bold text-black">{Math.round(score * 100)}</Text>
                    </View>
                </View>
                ...
                <View className="ml-5" style={{ gap: 16 }}>
                    {data.map((item, index) => (
                        <View key={index} className="flex-row items-center">
                            <Text>
                                <Text className="font-inter-medium text-base" style={{ color: item.color }}>
                                    {item.y}%{" "}
                                </Text>
                                <Text className="font-inter-light text-xs" style={{ color: item.color }}>
