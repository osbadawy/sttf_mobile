...
export default function StressSection({
  ...
  const isNearCenter =
    (markerAngle > 1.2 && markerAngle < 1.9) ||
    (markerAngle > 4.4 && markerAngle < 5.1);
  ...
  const adjustedTextX =
    textX +
    (isNearCenter
      ? centerAvoidanceDistance * Math.cos(markerAngle + Math.PI / 2)
      : 0);
  ...
  return (
    <CardWithTitle
      ...
      isRTL={isRTL}
    >
      <Text className={`w-full ${isRTL ? "text-right" : "text-left"}`}>
        {t("14DayAvg")}
      </Text>
        ...
        <Svg width={300} height={250}>
          ...
          <Circle
            ...
            strokeDashoffset={
              circumference *
              (removeFromCircle +
                (1 - stressPercentage) * (1 - removeFromCircle))
            }
            strokeLinecap="round"
            transform={`rotate(135 ${centerX} ${centerY})`}
          />
          ...
          <Polygon points={trianglePoints} fill="black" stroke="none" />
