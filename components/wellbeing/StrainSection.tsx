...
export default function StrainSection({strainToday, strain14Days}: StrainSectionProps) {
    ...
    const strainTodayXPos = getPositionOnLine(strainToday, 8);
    const strain14DaysXPos = getPositionOnLine(strain14Days, 1);
    ...
    return (
        <CardWithTitle title={tWellbeing('strain')} icon={<StrainIcon />} titleColor="text-black" arrow={false} isRTL={isRTL}>
           <View className="flex-row justify-between">
            ...
            <View className="w-full mt-2 relative justify-center" onLayout={(event) => {
                ...
                <View className={`absolute bg-strain w-5 h-5 rounded-full border-background border-2 border-solid`} style={{ left: strainTodayXPos }}/>
                <View className={`absolute bg-[#3C3C3C] w-2 h-6 border-background border-2 border-solid`} style={{ left: strain14DaysXPos }}/>
            </View>
            <View className="relative flex flex-row w-full mt-2">
                <Text className="effra-light" style={{ opacity: 0.7 }}>0</Text>
                <Text className="absolute effra-light " style={{ left: getPositionOnLine(10, 8), opacity: 0.7 }}>10</Text>
                <Text className="absolute effra-light" style={{ left: getPositionOnLine(14, 8), opacity: 0.7 }}>14</Text>
                <Text className="absolute effra-light" style={{ left: getPositionOnLine(18, 8), opacity: 0.7 }}>18</Text>
                <Text className="absolute effra-light" style={{ left: getPositionOnLine(21, 10), opacity: 0.7 }}>21</Text>
            </View>
        </CardWithTitle>
    );
}